...
import { Playground, ChatMessage, ModelConfig, PlaygroundDebugRequest, DebugSession } from '../types';
...
const PlaygroundDetail: React.FC = () => {
  ...
  // 获取 playground 详情
  const fetchPlayground = useCallback(async (playgroundId: number) => {
    try {
      setLoading(true);
      setError('');
  ...
  const startDebugSession = async (sessionId: number) => {
    ...
    try {
      ...
      // 调用 API 获取调试会话详情
      const debugSessionData = await playgroundsAPI.getDebugSession(playground.id, sessionId);
      ...
      // 如果有历史记录，初始化消息上下文
      if (debugSessionData && debugSessionData.records && Array.isArray(debugSessionData.records)) {
        ...
        // 处理历史记录，按轮次排序
        const sortedRecords = debugSessionData.records.sort((a: any, b: any) => a.turn_number - b.turn_number);
          ...
          try {
            ...
            if (record.response) {
              try {
                ...
                console.warn('解析响应数据失败:', e, '原始数据:', record.response);
              }
            }
          } catch (e) {
  ...
  if (error || !playground) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500">{error || '调试环境不存在'}</p>
        <div className="mt-6">
          <Link
            to="/playgrounds"
    ...
    <div className="space-y-6">
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span className="text-xs text-gray-500">
                    最后更新: {new Date(session.updated_at).toLocaleDateString()}
                  </span>
                  <Button
      ...
      {isDebugging ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">准备开始调试</h3>
