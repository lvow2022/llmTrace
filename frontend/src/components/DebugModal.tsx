...
const DebugModal: React.FC<DebugModalProps> = ({ isOpen, onClose, record }) => {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        if (data.data && Array.isArray(data.data)) {
          ...
          console.log('DebugModal: 使用直接结构，找到数据:', playgroundsArray.length, '条');
          ...
          console.log('DebugModal: 使用嵌套结构，找到数据:', playgroundsArray.length, '条');
        } else if (Array.isArray(data)) {
          // 数组结构：直接是数据数组
          playgroundsArray = data;
          console.log('DebugModal: 使用数组结构，找到数据:', playgroundsArray.length, '条');
        } else {
  ...
  useEffect(() => {
    if (isOpen) {
      fetchPlaygrounds();
      // 自动生成 Playground 名称
      setNewPlaygroundName(`调试_${record.session_id}_轮次${record.turn_number}`);
    }
  ...
  const handleStartDebug = async () => {
    ...
    try {
      ...
      const sessions = await playgroundsAPI.getPlaygroundSessions(selectedPlaygroundId);
      ...
      if (sessions.length > 0) {
        ...
        // 创建新的调试会话
        const debugSession = await recordsAPI.createDebugSessionFromRecord(record.id.toString(), {
      ...
      if (sessionId) {
        ...
        navigate(`/playground/${selectedPlaygroundId}/sessions/${sessionId}`);
      }
    } catch (err) {
  ...
  const handleCreateAndStart = async () => {
    try {
      ...
      navigate(`/playground/${newPlayground.id}/sessions/${debugSession.id}`);
    } catch (err) {
          ...
          {loading ? (
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto border border-gray-200 rounded-md p-3">
              {availablePlaygrounds.map(playground => (
                <label key={playground.id} className="flex items-center space-x-3 cursor-pointer hover:bg-gray-50 p-2 rounded">
                  <input
                    type="radio"
                    name="playground"
                    value={playground.id}
                    checked={selectedPlaygroundId === playground.id}
                    onChange={(e) => setSelectedPlaygroundId(parseInt(e.target.value))}
                    className="text-blue-600"
                  />
                  <div className="flex-1">
                    <div className="font-medium text-gray-900">{playground.name}</div>
                    <div className="text-sm text-gray-500">
                      {playground.description && `${playground.description} | `}
                      状态: {playground.status} | 
                      创建时间: {new Date(playground.created_at).toLocaleDateString()}
                    </div>
                  </div>
                </label>
              ))}
