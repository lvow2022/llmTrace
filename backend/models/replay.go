...
//func GetReplaySessionRecords(sessionID string, page, size int) ([]DebugRecord, int64, error) {
//	var total int64
//	if err := db.Model(&DebugRecord{}).Where("replay_session_id = ?", sessionID).Count(&total).Error; err != nil {
//		return nil, 0, fmt.Errorf("failed to count replay records: %v", err)
//	}
//
//	var replayRecords []DebugRecord
//	offset := (page - 1) * size
...
//
//	// 删除相关的重放记录
//	if err := tx.Where("replay_session_id = ?", sessionID).Delete(&DebugRecord{}).Error; err != nil {
//		tx.Rollback()
