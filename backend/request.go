...
type CreatePlaygroundRequest struct {
	OriginalRecordID string `json:"original_record_id" binding:"required"` // 基于哪个记录创建
	Name             string `json:"name"`                                  // Playground 名称
}
...
type PlaygroundSession struct {
	ID                 string    `json:"id" gorm:"primaryKey;type:varchar(255)"`
	Name               string    `json:"name" gorm:"type:varchar(255);not null"`
	OriginalRecordID   string    `json:"original_record_id" gorm:"type:varchar(255);not null;index"`
	OriginalSessionID  string    `json:"original_session_id" gorm:"type:varchar(255);not null;index"`
...
type PlaygroundRecord struct {
	...
	TestNumber          int       `json:"test_number" gorm:"not null"` // 测试次数
	Request             string    `json:"request" gorm:"type:text;not null"`
	Response            string    `json:"response" gorm:"type:text"`
	Status              string    `json:"status" gorm:"type:varchar(50);not null"`
	ErrorMsg            string    `json:"error_msg" gorm:"type:text"`
...
// PlaygroundTestRequest Playground 测试请求
type PlaygroundTestRequest struct {
	PlaygroundSessionID string      `json:"playground_session_id" binding:"required"`
	Request             interface{} `json:"request" binding:"required"`
